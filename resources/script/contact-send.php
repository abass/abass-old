...
if( isset($_POST['name']) && empty($_POST['spam-check']) ) {
	...
	if( strlen($input_name) < 2 ) $error['name'] = '<label for="contact-name">Please enter your <b>Name</b></label>';
	if( strlen($input_message) < 5 ) $error['message'] = '<label for="contact-message">Please leave a longer <b>Message</b></label>';

	// Make sure the email is valid
	if( !filter_var($input_email, FILTER_VALIDATE_EMAIL) ) $error['email'] = '<label for="contact-email">Please enter a valid <b>Email Address</b></label>';
	...
	if( !$error ) {
		...
		if( mail($email_address, $subject, $message, "From: $input_email") ) {
			echo '<p class="success"><b>EMAIL SENT SUCCESSFULLY.</b><br />' . "Dear $input_name, " . 'thank you for contacting me. Please allow for <b>24-48</b> hours for me to review your request and get back to you.<br /><br /><b>Please verify that this is your correct Email Address:</b><br />' . "Email Address: <i>$input_email</i>" . '<br /><br /><span class="red"><b>PLEASE NOTE:</b></span><br /> If I do not respond to your request within a reasonable amount of time, please give me a call as there may have been an error on our end with your request.</p>';
		} else {
			echo '<p class="error">There was a problem sending your email! Please contact me at alex@abass.co as there seems to be an error on our end with the form.</p>';
		}
